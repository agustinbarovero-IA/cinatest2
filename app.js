...
function renderAccionEstibaForm(tipoAccion){
  ...
  const tituloAccion = esLiberacion ? 'LIBERACION DE ESTIBA' : 'INTERVENCION DE ESTIBA';
  const verboAccion = esLiberacion ? 'Liberar' : 'Intervenir';
  const descripcionAccion = esLiberacion
  ...
  menuGrid.innerHTML = `
    <div class="intervencion-wrap">
      <section class="intervencion-card">
        <h2 class="intervencion-title">${verboAccion} estiba 65984-3</h2>
        <p class="intervencion-subtitle">${descripcionAccion}</p>
      </section>
      ...
      <section class="intervencion-card">
        ...
        <div class="intervencion-options">
          <label class="intervencion-option-row">
            ...
            <span class="intervencion-option-label">${verboAccion} estiba completa</span>
          </label>

          <label class="intervencion-option-row">
            <input type="radio" name="tipoAccionEstiba" value="estiba_parcial">
            <span class="intervencion-option-label">${verboAccion} estiba parcial</span>
            <input
              type="number"
              id="cantidadParcial"
          ...
          <label class="intervencion-option-row">
            ...
            <span class="intervencion-option-label">${verboAccion} entrada entera</span>
          </label>
        </div>
      </section>
      ...
      <section class="intervencion-card">
        <div class="intervencion-actions">
          <button id="btnConfirmarAccionEstiba" class="intervencion-submit">${verboAccion}</button>
        </div>
      </section>
    </div>
  `;
  ...
  const btnConfirmar = document.getElementById('btnConfirmarAccionEstiba');

  function updateCantidadParcial(){
    const selected = document.querySelector('input[name="tipoAccionEstiba"]:checked')?.value;
  ...
  btnConfirmar.addEventListener('click', () => {
    ...
    if(!motivo){
      alert(`Seleccione un motivo para ${esLiberacion ? 'la liberación' : 'la intervención'}.`);
      return;
    }
    ...
    if(tipo === 'estiba_parcial'){
      ...
      if(!cantidad || cantidad <= 0){
        alert(`Ingrese la cantidad para ${esLiberacion ? 'la liberación' : 'la intervención'} parcial.`);
        return;
      }
    }
...
function renderTemperaturasForm(){
  ...
  document.getElementById('btnCargarTemperaturas')?.addEventListener('click', () => {
    ...
    for(let i = 0; i < 10; i++){
      const producto = document.getElementById(`producto_${i}`)?.value || '';
      const t1 = document.getElementById(`temp1_${i}`)?.value || '';
      const t2 = document.getElementById(`temp2_${i}`)?.value || '';
      const tieneAlgo = producto || t1 || t2;

      if(tieneAlgo){
        if(!producto){
          alert(`Seleccione el producto en el registro ${i + 1}.`);
          return;
        }
        if(t1 === '' || t2 === ''){
          alert(`Complete las 2 temperaturas del registro ${i + 1}.`);
          return;
        }
        registrosValidos++;
      }
    ...
    if(registrosValidos === 0){
      alert('Cargue al menos un registro de temperatura.');
      return;
    }
...
userBtn.addEventListener('click', () => {
  openModule('https://sistema.cinafrio.com/intranet2/app.php/persona/306/show');
});
...
sinUbicacionBox.addEventListener('click', () => {
  openModule('https://sistema.cinafrio.com/intranet/index.php/estiba/sinPosicion');
});

vencidasBox.addEventListener('click', () => {
  openModule('https://sistema.cinafrio.com/intranet/index.php/infostock/vencimientos');
});
